...
usage() {
  printf '%s\n' "Usage: bash scripts/run_dcgan_experiments.sh [--epochs N] [train_dcgan args]"
  printf '%s\n' ""
  printf '%s\n' "Examples:"
  printf '%s\n' "  bash scripts/run_dcgan_experiments.sh"
  printf '%s\n' "  bash scripts/run_dcgan_experiments.sh --epochs 30"
  printf '%s\n' "  bash scripts/run_dcgan_experiments.sh --device cuda"
  printf '%s\n' ""
  printf '%s\n' "Set PYTHON_BIN to use a specific interpreter, for example:"
  printf '%s\n' "  PYTHON_BIN=./venv/bin/python bash scripts/run_dcgan_experiments.sh --epochs 30"
}
...
make_temp_config() {
  local config_path="$1"
  local epochs="$2"
  local tmp_dir="${TMPDIR:-/tmp}/dcgan_experiment_configs"
  local config_name
...
for config in "${CONFIGS[@]}"; do
  ...
  printf '\n%s\n' "============================================================"
  printf '%s\n' "Training DCGAN config: $config_to_run"
  printf '%s\n' "============================================================"
  "$PYTHON_BIN" -m src.train_dcgan --config "$config_to_run" "${TRAIN_ARGS[@]}"
done
