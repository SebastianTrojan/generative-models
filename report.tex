...
The best DCGAN result came from increasing the discriminator learning rate while keeping the baseline stabilisers active. This suggests that, under the tested setup, the generator benefited from a slightly stronger discriminator signal.
...
The best DDPM result came from increasing U-Net width. The baseline was already close to this result, while the cosine schedule, lower learning rate, disabled dropout, and longer diffusion chain all reduced performance in this setup. Although the 1000-step model uses a finer noising chain, it is also slower to sample and did not improve FID in this limited-compute setting.
...
To reduce collapse risk, the DCGAN baseline used label smoothing, instance noise, and separate generator/discriminator learning rates.
The sweep then tested related changes, including removing instance noise, removing label smoothing, changing the learning-rate balance, and adding minibatch-standard-deviation features.
Minibatch standard deviation did not improve FID in this experiment.
